...
class ImageOptimizer:
    ...
    def optimize_session_images(self, session: NoteSession, vault_path: Path):
        assets_dir = vault_path / self.config.get('vault.assets_folder')
        assets_dir.mkdir(parents=True, exist_ok=True)
        for page in session.pages:
            for diagram in page.diagram_regions:
                if diagram.label:
                    optimized = self._optimize_diagram(page.raw_image_path, diagram.bbox, assets_dir, diagram.label, session.filename_base)
    ...
    def _optimize_diagram(self, source: Path, bbox: dict, output_dir: Path, label: str, session_filename_base: str) -> Path:
        ...
        # New filename format: session_filename_base_label.png
        filename = f"{session_filename_base}_{label.replace('.', '_')}.png"
